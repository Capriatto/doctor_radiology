...
class doctor_appointment(osv.osv):
	...
	_columns = {
		'attentiont_radiology_id': fields.many2one('doctor.attentions.radiology', 'Attentiont', ondelete='restrict', readonly=True),
	}


	def create_attentiont_radiology(self, cr, uid, doctor_appointment, modelo_crear,  context={}):
		"""
	...
	def generate_attentiont(self, cr, uid, ids, context={}):
		...
		if self.pool.get('doctor.doctor').modulo_instalado(cr, uid, 'doctor_radiology', context=context):
			
			if tipo_historia == u'doctor_radiology':
				context['default_patient_id'] = context.get('patient_id')
				context['default_professional_id'] = profesional_id
				context['tipo_cita_id'] = tipo_cita
				context['default_patient_birth_date'] = doctor_appointment_variable.patient_id.birth_date
				...
				return {
					...
					'view_type': 'form',
					'view_mode': 'form',
					'res_model': 'doctor.attentions.radiology',
					'res_id': attentiont_id or False,
					'view_id': [view_id] or False,
					'type': 'ir.actions.act_window',
					'context' : context or None,
